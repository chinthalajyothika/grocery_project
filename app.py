...
def admincreate():
    if request.method=='POST':
        ...
        try:
            ...
            data1 = {'a_id': a_id,'name':admin ,'email': aemail, 'mobile': amobile, 'password': password}  # Modified the keys to use 'aemail' and 'amobile'
            subject = 'The confirmation link has been sent to your Email'  # Corrected the grammar
            body = f"Click the link to confirm: {url_for('aconfirm', token=token1(data1, salt=salt), _external=True)}"
            sendmail(to=aemail, subject=subject, body=body)  # Updated the 'to' parameter to use 'aemail'
            flash('Link has been sent to this Email')
...
def aconfirm(token):
    try:
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('insert into admindetails(admin_id,admin_name,admin_email,admin_mobile,password) values(%s,%s,%s,%s,%s)',[data['a_id'],data['name'],data['email'],data['mobile'],data['password']])
        mydb.commit()
        cursor.close()
        flash('Details Registered successfully')
        return redirect(url_for('adminlogin'))
...
@app.route('/viewparticularitem/<id1>')
def viewparticularitem(id1):
    print()
    cursor=mydb.cursor(buffered=True)
    cursor.execute('select  bin_to_uuid(item_id),item_name,dis,qyt,category,price,imgid from additems where item_id=uuid_to_bin(%s)',[id1])
    items=cursor.fetchall()
    print('===================================',items)
    return render_template('dashboard.html',items=items)
...
def search():
    if request.method=='POST':
        ...
        if pattern.match(name):
            cursor=mydb.cursor(buffered=True)
            cursor.execute('select bin_to_uuid(item_id),item_name,dis,qyt,category,price,imgid from additems where item_name LIKE %s', [name + '%'])
